	...
	<view class="content"> 
		...
		<view class="contpic" v-show="jieguosh"> 
			...
			<view class="txt">
				...
				<view class="border" @click="xx()">
					<image src="http://s1.catelyn.info/zjxcx/qcc/bankuai/你十年后会月入多少.png" mode=""></image> 
				</view>
		...
		<view class="contpic" v-show="jieguosh"> 
			...
			<view class="txt">
				<view class="sanjiao"></view> 
				<view class="border">
					<text>很少有人会对钱不在意，很少有人会对赚大钱不感兴趣。你期盼着十年后的自己变成富有大佬吗？想知道十年后你会月入多少？快来测测吧！</text>
	...
	export default {
		data() {
			return {
				...
				avatarUrl:'',
				nickName:'',
				subject:[{"tltie":"你对自己的未来有很明确的规划吗？","text":["有，非常明确","没有随心所欲","有大致的方向，还在继续探索"],"checked":true},{"tltie":"十年后你更想住在哪里？","text":["城市中心","二环开外","郊区地带"],"checked":true},{"tltie":"假如你买了新房，你会对哪里进行最重点和细致的装修？","text":["厨房","我是","客厅","洗手间","书房","其他"],"checked":true},{"tltie":"自己能够独立后，你会愿意和家人住一起吗？","text":["非常愿意，不想离开家人","随便吧怎样都可以","不愿意，需要有些距离"],"checked":true}],
				list:[],
				radiotext:'',//暂存点击题目文字
				radiotop:'',//滚动高度
				jieguo:[]
			...
			radioclick(index){ 
				if(this.radiotext!=''){
					...
					if(index==this.subject.length-1){
						this.time=true
						this.list[index].checked=false
						let num=parseInt(Math.random()*1000)%10
						this.jieguopic='http://s1.catelyn.info/zjxcx/qcc/jieguo/你十年后会月入多少'+(num+1)+'.png'
						uni.pageScrollTo({
						    scrollTop: 9999,
						    duration: 300
						});
