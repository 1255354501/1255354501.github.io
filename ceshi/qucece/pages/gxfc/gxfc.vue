	...
	<view class="content"> 
		...
		<view class="contpic" v-show="jieguosh"> 
			...
			<view class="txt">
				...
				<view class="border" @click="xx()">
					<image src="http://s1.catelyn.info/zjxcx/qcc/bankuai/个性反差有多大.png" mode=""></image> 
				</view>
		...
		<view class="contpic" v-show="jieguosh"> 
			...
			<view class="txt">
				<view class="sanjiao"></view> 
				<view class="border">
					<text>每个人在外面对外界的时候，都会穿上一层保护衣，你外在表楼的样子难道真的是你自己吗？来测一下就知道了！</text>
		...
		data() {
			return {
				...
				subject:[
					...
					{
						tltie:'下面那个词更符合对你的形容？',
						text:['坚强的','固执的','温和的','乐观的','理智的','正义的','感性的'],
						checked:true
					},
					{
						tltie:'遇到不开心的事，你会抑制自己发泄吗？',
						text:['会','不会'],
						checked:true
					}, 
					{
						tltie:'对于第一次见面的人你会：',
						text:['侃侃而谈','比较拘谨','视眼缘而定'],
						checked:true
					}, 
					{
						tltie:'你的同性朋友多还是异性朋友多？：',
						text:['同性','异性','都差不多'],
						checked:true
					}
				],
			...
			radioclick(index){ 
				if(this.radiotext!=''){
					...
					if(index==this.subject.length-1){
						...
						this.jieguopic='http://s1.catelyn.info/zjxcx/qcc/jieguo/测测你的内外个性反差有多大？'+(num+1)+'.png'
						uni.pageScrollTo({
						    scrollTop:9999,
						    duration: 300
						});
