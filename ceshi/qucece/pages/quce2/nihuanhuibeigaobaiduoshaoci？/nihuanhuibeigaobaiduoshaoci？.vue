	...
	<view class="content"> 
		...
		<view class="contpic" v-show="jieguosh"> 
			...
			<view class="txt">
				...
				<view class="border" @click="xx()">
					<image src="http://s1.catelyn.info/zjxcx/qcc/bankuai/你还会被告白多少次？.jpg" mode=""></image> 
				</view>
		...
		<view class="contpic" v-show="jieguosh"> 
			...
			<view class="txt">
				<view class="sanjiao"></view> 
				<view class="border">
					<text>每个人或多或少在成长道路上都会被ta人表报，想知道你未来还会被表白多少次吗？快来测测吧~</text>
	...
	export default {
		data() {
			return {
				...
				nickName:'',
				subject:[
					{"tltie":"你生日的时候，更想和谁再一次过",
					"text":['朋友','家人','爱人','一个人'],"checked":true},
					{"tltie":"你更希望收到什么礼物？","text":['自己刚需要的东西','自己买不起的东西','浪漫的东西'],"checked":true},
					{"tltie":"最好的朋友忘记了你的生日，你会？","text":['难过','谅解ta','不在意'],"checked":true},
					{"tltie":"你认为生日许愿的愿望会实现吗？","text":['会','不会','也许会'],"checked":true}],
				list:[],
				radiotext:'',//暂存点击题目文字
				radiotop:'',//滚动高度
				jieguo:[]
			...
			radioclick(index){ 
				if(this.radiotext!=''){
					...
					if(index==this.subject.length-1){
						...
						this.list[index].checked=false
						let num=parseInt(Math.random()*1000)%2
						this.jieguopic='http://s1.catelyn.info/zjxcx/qcc/jieguo/你还会被告白多少次？'+(num+1)+'.png'
						uni.pageScrollTo({
						    scrollTop: 9999,
						    duration: 300
						});
